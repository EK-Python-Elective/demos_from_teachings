...
have to type violations live. The `pyproject.toml` here makes the folder a tiny
uv project — `ruff` and `pyright` pinned to the fork's versions, `[tool.ruff]` /
`[tool.pyright]` mirroring the fork's config — so `uv sync` once, then:
...
- [`pyright_type_errors.py`](pyright_type_errors.py) — eight deliberate type
  ...
  ```console
  $ uv run pyright             # 8 errors, standard mode
  ```
