...
The `pyproject.toml` next to this file makes the folder a tiny uv project with
`ruff` pinned to the fork's version and `[tool.ruff]` mirroring the fork's config
(only `include` differs). So, from this folder:

    uv sync
    uv run ruff check .          # 17 hits across the 14 rules
    uv run ruff check --fix .    # most auto-fix; 6 don't:
                                 # PLR2004, ANN x2, B006, RUF012, B905

Dropping this file straight into your fork works too — it already has the config
and the pinned tools, so `uv run ruff check ruff_check_failures.py` there.
"""
